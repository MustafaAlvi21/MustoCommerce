module.exports = { 
    findProductDataForWishlist: async function(req, res, next) {
        const wishlistDataModel = require('../models/wishlist')
        const productDataModel = require('../models/products')
        
        console.log('data -=-> 1' )
        let cartData = [];
        await wishlistDataModel.find({userId : req.user._id}).then( (data) => {        
            ...
            res.write('<h1> Can not find any data of wishlist for this user</h1>');
            res.end();
        })
    }
}